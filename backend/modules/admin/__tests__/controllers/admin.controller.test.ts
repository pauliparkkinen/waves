...
function makeService(overrides: Partial<IAdminService> = {}): IAdminService {
  return {
    ...
    createCollection: vi.fn().mockReturnValue({ collection_id: 'c-1', collection_permissions: [] }),
...
describe('createAdminRouter', () => {
  ...
  describe('GET /collections', () => {
    describe('given no authenticated user', () => {
      it('when the request is made, then it returns 401', async () => {
        const { createAdminRouter } = await import('../../controllers/admin.controller.js');
        const app = appWithUser(null).route('/', createAdminRouter(makeService()));

        const res = await app.request('http://localhost/collections');
    ...
    describe('given an authenticated user without admin:manage', () => {
      it('when the request is made, then it returns 403', async () => {
        ...
        const app = appWithUser(user).route('/', createAdminRouter(makeService()));
    ...
    describe('given an authenticated user with admin:manage', () => {
      it('when the request is made, then it returns 200 with the collections', async () => {
        ...
        expect(service.listCollections).toHaveBeenCalled();
      });
    });
  });
  ...
  describe('POST /collections', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when a valid body is sent, then it returns 201 with the created collection', async () => {
        ...
        expect(service.createCollection).toHaveBeenCalledWith({ collection_permissions: [] });
      });
    });
  });
  ...
  describe('GET /collections/:id', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when the collection exists, then it returns 200 with the collection', async () => {
        ...
        const res = await app.request('http://localhost/collections/c-1');
        const body = await res.json();

        expect(res.status).toBe(200);
        expect(body).toEqual(collection);
        expect(service.getCollection).toHaveBeenCalledWith('c-1');
      });
      ...
      it('when the collection does not exist, then it returns 404', async () => {
        ...
        const res = await app.request('http://localhost/collections/nonexistent');
  ...
  describe('DELETE /collections/:id', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when the collection exists, then it returns 200 with success', async () => {
        ...
        const res = await app.request('http://localhost/collections/c-1', { method: 'DELETE' });
        const body = await res.json();
  ...
  describe('GET /questions', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when called, then it returns the questions', async () => {
        const { createAdminRouter } = await import('../../controllers/admin.controller.js');
        const user: AuthUser = { sub: 'u-1', permissions: ['admin:manage'] };
        const questions = [
          {
        ...
        const res = await app.request('http://localhost/questions?collectionId=col-1');
        const body = await res.json();

        expect(res.status).toBe(200);
        expect(body).toEqual(questions);
        expect(service.listQuestions).toHaveBeenCalledWith('col-1');
      });
    });
  });
  ...
  describe('POST /questions', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when a valid body is sent, then it returns 201', async () => {
        ...
        const res = await app.request('http://localhost/questions', {
          method: 'POST',
          body: JSON.stringify({
            collection_id: 'col-1',
            question_symbol: 'q1',
  ...
  describe('GET /questions/:id', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when the question exists, then it returns 200 with the question', async () => {
        ...
        const res = await app.request('http://localhost/questions/q-1');
        const body = await res.json();

        expect(res.status).toBe(200);
        expect(body).toEqual(question);
        expect(service.getQuestion).toHaveBeenCalledWith('q-1');
      });
      ...
      it('when the question does not exist, then it returns 404', async () => {
        ...
        const res = await app.request('http://localhost/questions/nonexistent');
  ...
  describe('PUT /questions/:id', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when the question exists, then it returns 200 with the updated question', async () => {
        const { createAdminRouter } = await import('../../controllers/admin.controller.js');
        const user: AuthUser = { sub: 'u-1', permissions: ['admin:manage'] };
        const updated = {
          question_id: 'q-1',
  ...
  describe('DELETE /questions/:id', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when the question exists, then it returns 200 with success', async () => {
        ...
        const res = await app.request('http://localhost/questions/q-1', { method: 'DELETE' });
        const body = await res.json();
  ...
  describe('GET /sections', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when called, then it returns the sections', async () => {
        const { createAdminRouter } = await import('../../controllers/admin.controller.js');
        const user: AuthUser = { sub: 'u-1', permissions: ['admin:manage'] };
        const sections = [
          {
        ...
        const res = await app.request('http://localhost/sections');
        const body = await res.json();
  ...
  describe('GET /sections/:id', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when the section exists, then it returns 200 with the section', async () => {
        ...
        const res = await app.request('http://localhost/sections/s-1');
        const body = await res.json();

        expect(res.status).toBe(200);
        expect(body).toEqual(section);
        expect(service.getSection).toHaveBeenCalledWith('s-1');
      });
      ...
      it('when the section does not exist, then it returns 404', async () => {
        ...
        const res = await app.request('http://localhost/sections/nonexistent');
  ...
  describe('PUT /sections/:id', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when the section exists, then it returns 200 with the updated section', async () => {
        const { createAdminRouter } = await import('../../controllers/admin.controller.js');
        const user: AuthUser = { sub: 'u-1', permissions: ['admin:manage'] };
        const updated = {
          section_id: 's-1',
          section_symbol: 's1-updated',
          version: 1,
          status: 'draft',
          section_questions: [],
          translations: [],
        ...
        const res = await app.request('http://localhost/sections/s-1', {
          method: 'PUT',
          body: JSON.stringify({ section_symbol: 's1-updated' }),
          headers: { 'Content-Type': 'application/json' },
        });
        const body = await res.json();
  ...
  describe('DELETE /sections/:id', () => {
    describe('given an authenticated user with admin:manage', () => {
      it('when the section exists, then it returns 200 with success', async () => {
        ...
        const res = await app.request('http://localhost/sections/s-1', { method: 'DELETE' });
        const body = await res.json();
